...
function checkPlanSize() {
  case "$1" in
    fixedminimum|small|medium|large)
...
function usage {
    echo $0: usage: $0 [-h] -n namespace -N operator_namespace [-t tethered_namespace] -f file_storageclass [-b block_storageclass] [-p plan_size] -C database_provider -H database_host -D database_name -U database_user -P database_password -L database_port[-X ca_username][-Y ca_password] -V addon_version [-T provision_timeout][-R][-S cert_file][-i dbserver_id][-j dbserver_pass][-c crypto_conformance]
}
...
while getopts ":hn:t:N:f:b:p:d:C:H:D:U:i:j:L:P:X:Y:V:T:RS:c:" opt; do
     case ${opt} in
     n)
        namespace=$OPTARG
        ;;
...
if [ -z "$operator_namespace" ]; then
  usageError "Please provide an operator namespace."
fi
if [ -z "$namespace" ]; then
...
if [ -z ${ca_username} ] && [ -z ${ca_password} ]; then
  ...
  isIAMEnabled=$(oc get zenservice lite-cr -n ${namespace} -o jsonpath={.spec.iamIntegration})
...
echo "Openshift client version information:"
oc version
...
loginRequestResponse=$(curl -k -c ${zen_cookie_file} "${cpd_web_url}/v1/preauth/signin?context=zen" \
...
if [[ ${remove_running_instance} == "true" ]]; then
  ...
  get_status_response=$(curl -v -k -H "Content-Type:application/json" -H "Authorization: Bearer ${bearer_auth_token}" -b ${zen_cookie_file} -X GET ${cpd_web_url}/zen-data/v3/service_instances?addon_type=cognos-analytics-app)
#  echo "get_status_response: ${get_status_response}"
  ...
  function instanceStatus(){
    instance_status_response=$(curl -v -k -H "Content-Type:application/json" -H "Authorization: Bearer ${bearer_auth_token}" -b ${zen_cookie_file} -X GET ${cpd_web_url}/zen-data/v3/service_instances/${instance_id}/?include_service_status=true)
  ...
  if [[ ${total_count} != 0 ]] ; then
      ...
      while [ ${currInstance} -lt ${total_count} ]; do
        ...
        curl -v -k -H "Content-Type:application/json" -H "Authorization: Bearer ${bearer_auth_token}" -b ${zen_cookie_file} -X DELETE ${cpd_web_url}/zen-data/v3/service_instances/${instance_id}
...
provisioningRequestResponse=$(curl -v -k -H "Content-Type:application/json" \
      -b ${zen_cookie_file} -X POST -d "$(provision_data)" ${cpd_web_url}/zen-data/v3/service_instances)
...
sleep 30
curl -v -k -b ${zen_cookie_file} -H "Content-Type:application/json" \
...
while [ ${minutesRemainingInTimeOut} -gt 0 ]; do
  ...
  statusJSON=$(curl -k -b ${zen_cookie_file} -H "Content-Type:application/json" \
